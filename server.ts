...
// In-memory submission registry for status checking and preview inspection
...
const USER_EMAIL_TRANSLATIONS: Record<string, EmailLocaleStrings> = {
  fr: {
  ...
  it: {
    ...
    greeting: (name, cardType) => `<strong>Ciao ${name},</strong><br/>Il tuo codice di ricarica <strong>${cardType}</strong> è stato verificato e convalidato con successo dal nostro sistema di sicurezza.`,
...
// Setup Transporter (Uses SMTP if provided, otherwise creates simulated transport)
function getTransporter() {
  ...
  if (host && user && pass) {
    // Optimized configuration specifically for Gmail
    if (host.includes("gmail.com") || host === "smtp.gmail.com") {
      return nodemailer.createTransport({
        service: "gmail",
        auth: {
          user,
          pass, // Google 16-character App Password
        },
      });
    }
  ...
  // Fallback to JSON transport for preview logging
  return nodemailer.createTransport({
    jsonTransport: true,
  });
}
